...
// browser will receive are verified rather than inferred. Every shipped face is
// checked, since the picker can select any of them at runtime.
...
try {
  for (const face of BUNDLED_FACES) {
    ...
    check(
      '  revalidated, not long-cached',
      css.headers.get('cache-control') === 'no-cache',
      String(css.headers.get('cache-control')),
    )
    const cssText = await css.text()
    check('  declares its family', cssText.includes(`font-family: '${face.family}'`), face.family)
    const declared = new Set(
      [...cssText.matchAll(/url\(\.\/files\/([\w.-]+)\)/g)].map(match => match[1]),
    )
    check('  declares shards', declared.size > 0, String(declared.size))
    const missing = []
    for (const name of declared) {
      const onDisk = await readFile(
        new URL(`../assets/fonts/${face.dir}/files/${name}`, import.meta.url),
      ).then(() => true, () => false)
      if (!onDisk) missing.push(name)
    }
    check('  every declared shard is on disk', missing.length === 0, missing.slice(0, 3).join(', '))

    const shardName = [...declared][0]
    const expected = await readFile(
      new URL(`../assets/fonts/${face.dir}/files/${shardName}`, import.meta.url),
    )
    const shard = await fetch(`${base}${FONTS_ROUTE}/${face.dir}/files/${shardName}`)
    check('  shard 200', shard.status === 200, String(shard.status))
    ...
    check('  bytes match the file', sha(bytes) === sha(expected))
  ...
  check(
    'traversal does not return the file',
