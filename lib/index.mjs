...
const FONT_IDS = FONT_FACES.map((face) => face.id);
/**
...
*/
function fontFaceById(id) {
	return FONT_FACES.find((face) => face.id === id) ?? FONT_FACES.find((face) => face.id === "noto-sans-sc") ?? {
		id: "noto-sans-sc",
		dir: "noto-sans-sc",
		family: "sans-serif"
	};
...
const FONT_SETTINGS_SCHEMA = z.object({ font: z.string().default(DEFAULT_FONT_ID) });
/**
...
*/
function registerFontSettings(ctx) {
	ctx.inject(["settings"], (settingsCtx) => {
		const provider = settingsCtx.settings;
		try {
			provider.register(FONT_SETTINGS_NS, FONT_SETTINGS_SCHEMA, {
...
//#endregion
export { DEFAULT_FONT_ID, FONTS_ROUTE, FONT_FACES, FONT_IDS, FONT_SETTINGS_NS, FONT_SETTINGS_SCHEMA, apply, fontFaceById, fontFileFor, fontStack, inject, name, serveFontFile };
