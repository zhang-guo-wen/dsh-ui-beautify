	...
	factory: (require) => {
		...
		*/
		function resolveMotionChoice(id) {
			return MOTION_CHOICE_IDS.find((choice) => choice === id) ?? "system";
		}
		/**
		...
		*/
		const SPRINT_SPANS_PER_SECOND = .5;
		/**
		...
		*/
		...
		const css$1 = ".wtM5EW_lane{width:calc(100% - var(--dsh-composer-side-clearance) - var(--dsh-composer-side-clearance) - var(--dsh-composer-dock-inset) - var(--dsh-composer-dock-inset));max-width:calc(var(--dsh-composer-card-max-width) - var(--dsh-composer-dock-inset) - var(--dsh-composer-dock-inset));height:26px;color:var(--dsw-alias-label-tertiary);pointer-events:none;margin:0 auto;position:relative;overflow:hidden}.wtM5EW_rider{will-change:transform;width:47px;height:26px;position:absolute;top:0;left:0}.wtM5EW_bike{fill:none;stroke:currentColor;stroke-width:2.4px;stroke-linecap:round;stroke-linejoin:round;width:47px;height:26px;display:block}.wtM5EW_wheel{stroke-width:1.8px}.wtM5EW_frame{stroke-width:2.4px}.wtM5EW_riderBody{stroke-width:3.2px}.wtM5EW_crank{stroke-width:1.6px}.wtM5EW_leg{stroke-width:2.4px}.wtM5EW_legFar{opacity:.5}";
		...
		*/
		...
		const WHEEL_RADIUS_PX = 13 * Math.min(BIKE_WIDTH_PX / VIEWBOX_WIDTH, 26 / VIEWBOX_HEIGHT);
		/**
		...
		* @returns the cyclist.
		*/
		const Artwork = (0, react.memo)(function Artwork({ rearWheel, frontWheel, nearLeg, farLeg }) {
		...
		*/
		function BikeLane({ useChat, useBeautify }) {
			...
			(0, react.useEffect)(() => {
				...
				const step = (now) => {
					const elapsed = Math.min((now - state.previous) / 1e3, MAX_FRAME_SECONDS);
					state.previous = now;
					...
					if (traverse > BIKE_WIDTH_PX) {
						...
						if (state.speed > 0) {
							...
							riderElement.style.transform = `translate3d(${state.progress * traverse - BIKE_WIDTH_PX}px, 0, 0)`;
							...
							const phase = -(state.rotation * Math.PI) / 540;
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					...
					ref: rider,
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Artwork, {
						rearWheel,
						frontWheel,
						nearLeg,
						farLeg
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:C:\02-codespace\deepseek-harness\dsh-ui-beautify\src\client\SettingRow.module.css.mjs
		const css = ".BqWVFa_row{border-bottom:.5px solid var(--dsw-alias-border-l2);align-items:center;gap:8px;padding:16px 0;display:flex}.BqWVFa_rowText{flex-direction:column;flex:1;gap:4px;min-width:0;padding-right:48px;display:flex}.BqWVFa_title{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:400;line-height:22px}.BqWVFa_desc{color:var(--dsw-alias-label-tertiary);font-size:12px;font-weight:400;line-height:18px}.BqWVFa_meta{font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-tertiary);opacity:.75;font-size:12px;font-weight:400;line-height:18px}.BqWVFa_meta:empty{display:none}.BqWVFa_selector{background:var(--dsw-alias-bg-module-platform);height:36px;font:inherit;color:var(--dsw-alias-label-primary);cursor:pointer;border:none;border-radius:18px;align-items:center;gap:12px;padding:0 14px;font-size:14px;line-height:22px;display:inline-flex}.BqWVFa_selector:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover)}.BqWVFa_selector:disabled{cursor:default;opacity:.6}.BqWVFa_chevron{flex:none}";
		const tagId = "@guowenzhang/dsh-ui-beautify/SettingRow.module.css";
		...
		var SettingRow_module_css_default = {
			"chevron": "BqWVFa_chevron",
			"desc": "BqWVFa_desc",
			"meta": "BqWVFa_meta",
			"row": "BqWVFa_row",
			"rowText": "BqWVFa_rowText",
			"selector": "BqWVFa_selector",
			"title": "BqWVFa_title"
		};
		//#endregion
		...
		*/
		...
		function copyFor(role, id) {
			return id === "system" ? SYSTEM_COPY[role] : CHOICE_COPY$1[id];
		}
		/**
		...
		*/
		function detailLine$1(t, role, state) {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							children: detailLine$1(t, role, state)
						})
					]
		...
		*/
		...
		const CHOICE_COPY = {
			system: {
				name: "motionSystem",
				desc: "motionSystemDesc"
			},
			always: {
				name: "motionAlways",
				desc: "motionAlwaysDesc"
			},
			off: {
				name: "motionOff",
				desc: "motionOffDesc"
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					anchor: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						...
						children: [t(selected.name), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconChevronDownOutlineRegular, { className: SettingRow_module_css_default.chevron })]
					})
				})]
			});
		}
