window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const tagId = "@zhang-guo-wen/dsh-ui-beautify/FontSection.module.css";
		...
		/**
		* Copy keys per face id. The registry in `fonts.ts` owns the ids and families;
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							children: FONT_FACES.map((face) => {
								const copy = FACE_COPY[face.id];
								if (copy === void 0) return null;
								const active = face.id === state.font;
			...
			*/
			...
			projection() {
				...
				return {
					...
					font: snapshot.value?.font ?? "noto-sans-sc"
				};
			}
			publish() {
		...
		*/
		function apply(ctx) {
			...
			const scope = ctx.settingsScope.bind({ namespace: FONT_SETTINGS_NS });
			const controller = new FontController(scope);
			ctx.effect(() => () => {
				controller.dispose();
			}, "ui-beautify: settings scope");
